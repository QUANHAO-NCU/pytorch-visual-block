...
def load_dataset(dataset, class_number, mean, std, hyper_para):
    if dataset == 'abnormal':
        ...
        normal_data_path = '/home/labuser/Desktop/research/datasets/anomaly/normal/mat/raw/anomaly_normal_data_' + str(
            class_number) + '.mat'
        abnormal_data_path = '/home/labuser/Desktop/research/datasets/anomaly/abnormal/mat/raw/anomaly_abnormal_data_' + str(
            class_number) + '.mat'
...
def choose_classifier(dataset, class_number, model_type, model, classifier, D, hyper_para, train_data, test_data,
                      test_label, no_train_data, no_test_data, inm, relu, m, s):
    if hyper_para.verbose:
        print('Extracting features.....')

    train_features = np.memmap('../../temp_files/train_features_temp.bin', dtype='float32', mode='w+',
                               shape=(no_train_data, hyper_para.D))
    ...
    if (hyper_para.classifier_type == 'OC_CNN'):
        ...
        for j in range(no_test_data):
            temp = model(AddNoise(torch.autograd.Variable(test_data[j:(j + 1)].cuda().contiguous().float()),
                                  hyper_para.sigma1)).float()
            temp = temp.view(1, 1, hyper_para.D)
            temp = inm(temp)
...
def choose_method(dataset, model_type, class_number, hyper_para):
    ...
    if hyper_para.method == 'OC_CNN':
        ...
        raise argparse.ArgumentTypeError(
            'model_type argument can be only one of these OC_CNN, OC_SVM_linear, Bi_SVM_linear')
...
def OC_CNN(dataset, model_type, class_number, hyper_para):
    ...
    for i in range(hyper_para.iterations):
        ...
        # get the labels
        labels = np.concatenate((np.zeros((int(hyper_para.batch_size),)), np.ones((int(hyper_para.batch_size),))),
                                axis=0)
        ...
        if (hyper_para.verbose == True):
            if (i % (hyper_para.stats_freq) == (hyper_para.stats_freq - 1)):  # print every stats_frequency batches
                line = hyper_para.BLUE + '[' + str(format(i + 1, '8d')) + '/' + str(
                    ...
                    format(running_loss / hyper_para.stats_freq, '1.8f')) + \
                       hyper_para.GREEN + ' cc: ' + hyper_para.ENDC + str(
                    format(running_cc / hyper_para.stats_freq, '1.8f'))
                print(line)
                running_loss = 0.0
                running_cc = 0.0
    ...
    torch.save(model,
               '../../save_folder/saved_models/' + dataset + '/model/' + str(class_number) + '/' + model_type + '_' +
               str(hyper_para.iterations) + '_' +
               str(hyper_para.lr) + '_' +
               str(hyper_para.sigma) + '.pth')

    torch.save(model, '../../save_folder/saved_models/' + dataset + '/classifier/' + str(
        class_number) + '/' + model_type + '_' +
               str(hyper_para.iterations) + '_' +
               str(hyper_para.lr) + '_' +
               str(hyper_para.sigma) + '.pth')

    scipy.io.savemat(
        '../../save_folder/results/' + dataset + '/' + str(class_number) + '/' + model_type + '_OCCNN123_' +
...
def OC_SVM_linear(dataset, model_type, class_number, hyper_para):
    ...
    for i in range(no_train_data):
        train_features[i:(i + 1)] = (model(torch.autograd.Variable(train_data[i:(i + 1)].cuda().contiguous().float(),
                                                                   volatile=True)).float()).data.cpu()
    train_data = None
    ...
    joblib.dump(oc_svm_clf, '../../save_folder/saved_models/' + dataset + '/classifier/' + str(
        class_number) + '/' + model_type + '_OCSVMlin_' + str(hyper_para.N) + '.pkl')

    scipy.io.savemat(
        '../../save_folder/results/' + dataset + '/' + str(class_number) + '/' + model_type + '_OCSVMlin_' + str(
            hyper_para.N) + '.mat',
        {'train_features': train_features, 'test_features': test_features, 'test_label': test_label,
...
def Bi_SVM_linear(dataset, model_type, class_number, hyper_para):
    ...
    joblib.dump(oc_svm_clf, '../../save_folder/saved_models/' + dataset + '/classifier/' + str(
        class_number) + '/' + model_type + '_BiSVMlin_' + str(hyper_para.N) + '.pkl')

    scipy.io.savemat(
        '../../save_folder/results/' + dataset + '/' + str(class_number) + '/' + model_type + '_BiSVMlin_' + str(
            hyper_para.N) + '.mat',
        {'auc': area_under_curve, 'train_features': train_features, 'test_scores': test_scores,
         'test_features': test_features, 'test_label': test_label})
